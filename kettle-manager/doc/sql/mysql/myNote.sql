...
ALTER TABLE `r_job`
ADD COLUMN `PROJECT_CODE`  varchar(200) NULL DEFAULT '' COMMENT '运行在' AFTER `REPOSITORY_CODE`;
ALTER TABLE `r_job`
ADD COLUMN `OORDER`  decimal(65,0) NULL DEFAULT '9999' COMMENT '对象排序' AFTER `PROJECT_CODE`;
ALTER TABLE `r_job`
ADD COLUMN `LOG_LEVEL`  varchar(200) NULL DEFAULT '3' COMMENT '日志级别' AFTER `OORDER`;
...
CREATE TABLE `job_warning` (
  ...
  `JOB_NAME` varchar(200) DEFAULT NULL COMMENT '作业名称',
  `LOG_FILE` text COMMENT '日志文件',
  `MSG` text COMMENT '预警日志'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='作业预警';
